...
|---|---|---|---|---|
| 1 | BLOCKER | Корректность | `BLOCKING` + `cascade=NONE` приводит к удалению дочерней сущности | `DeletionService.php:288` → `Service/DeletionOrchestrator.php:111` |
...
**Часть 2 задания** — юнит-тесты в [`tests/`](tests/); что именно ими подтверждено, описано в §1.2.
...
**Направление атрибута выбрано верно.** `RelationTo` объявляется на дочерней сущности (в терминах
README — на «ребёнке») и указывает на родителя (`Attribute/RelationTo.php:10-21`) — единственный
работоспособный вариант: иначе каждый
...
#### 1. [BLOCKER] Связь `BLOCKING` с `cascade=NONE` приводит к удалению дочерней сущности

`Deletion/DeletionService.php:288` → `Deletion/Service/DeletionOrchestrator.php:111`

**Проблема.** `findChildrenByAttributes()` кладёт дочерние сущности с `BLOCKING` в `childrenDelete` через ветку
...
|---|---|
| `Enum/RelationType.php:9` | блокируется удаление **текущей** сущности (аннотированной дочерней сущности) |
...
| `DeletionService.php:288-292` | реализует комментарий: дочерняя сущность с `BLOCKING` блокирует родителя |
...
/** Наличие дочерних записей запрещает удалить РОДИТЕЛЯ ($entity). Аналог ON DELETE RESTRICT. */
...
на первом же скопированном примере. Ошибка возникает при проверке **дочерней сущности**, поэтому пример
...
выдаёт `[ChainChild, CascadeGrandchild]` — родителя раньше дочерней сущности. Само нарушение FK-констрейнта
...
private function buildRecursive(object $parent, RelationsDto $relations, array &$deleteMap, array &$detach, array &$visited = [], int $depth = 0): void
{
    ...
    foreach ($group->ids as $cid) {
        ...
        $visited[$key] = true;
        // ... существующая загрузка дочерней сущности и рекурсия, с $visited и $depth + 1
    }
}
```
...
`$group->field`. Но `$group->ids` — это идентификаторы дочерних сущностей (`DeletionService.php:266-280`), а
`$group->field` — имя поля дочерней сущности, ссылающегося на **родителя** (`RelationTo.php:14`). Отдельно
...
Хуже того, фолбэк `findOneBy([$group->field => $cid])` (`:130-131`) ищет дочернюю сущность, у которой FK на
родителя равен идентификатору **другой** дочерней сущности: при пересечении диапазонов он вернёт постороннюю,
...
никогда, и FK дочерней записи не зануляется. Так как detach по осознанному решению авторов
...
другого модуля и единственный источник данных о дочерних сущностях (`:261,271,277`). Абстракции нет. Вторая
...
`field` есть у дочерней сущности. Опечатка в FQCN создаёт ключ, который никто никогда не читает — связь тихо
...
Компромисс: доменные события по удалённым дочерним записям не выпустить, soft-delete через каскад БД не
выразить, `dryRun` считать отдельно; схема становится частью бизнес-логики.

Текущий подход прав, если на удаление дочерних сущностей навешаны побочные эффекты — списание с баланса, очистка
...
Сейчас `buildRecursive()` грузит каждую дочернюю сущность (`DeletionOrchestrator.php:129`), чтобы передать в
...
может утянуть недоступные дочерние сущности, а план уже является данными (`OrderedPlanDto`) и передаётся воутеру
...
1. **Какая из двух трактовок `BLOCKING` авторитетна?** Докблок `Enum/RelationType.php:9` и README
   говорят «блокируется удаление аннотированной дочерней сущности», код
